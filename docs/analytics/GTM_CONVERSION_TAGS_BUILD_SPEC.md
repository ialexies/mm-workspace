...
- **GA4 Key Events** — `purchase_all_in` and `purchase_parent_voucher` have both fired for real (confirmed
  in Realtime/live network requests, 2026-08-13) but neither had appeared in the Admin → Events list to be
  starred as of this check — that list lags behind actual firing by an unknown amount, not by design, just
  needs time. Check back before assuming either is still un-starred. The other 3 new event names
  (`purchase_hostel`, `purchase_tour`, `purchase_surf_camp`) don't exist yet since those tags aren't built.
...
live and verified" above is based on), and the **2026-08-13 8:24 PM publish** ("Add Parent Voucher purchase
tracking (GA4 + Meta + TikTok)", 39 tags / 34 triggers / 87 variables) that Parent Voucher's "live and
verified" status above is based on. If you need to diff current-vs-this-doc again, re-export the live
workspace/version from GTM rather than looking for any of these in the repo — none of them are there.
