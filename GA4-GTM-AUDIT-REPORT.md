...
- **Multi-property `purchase` items dropped + `item_category3` (property name) added** (2026-07-16) — ✅ Merged to `fix/purchase-item-category3-multi-property` (frontend). A real order spanning two properties (via the "add a stop" checkout upsell) reported the correct total `value` but `items[]` only included the anchor property's rooms — the second property's revenue was invisible at the item level, and item-level revenue never reconciled with the transaction total for these orders. Fixed by rebuilding `purchase`'s `lineItems` from the confirmation response's `bookings[]` (one entry per property) instead of the flat top-level summary. Also added `item_category3` (property/hostel name, e.g. `"Mad Monkey Dumaguete"`) to `begin_checkout`/`purchase` for both rooms and tours — a standard GA4 field, no dimension registration needed, usable in Explore immediately. Verified live on localhost: a real 2-property order correctly split into 2 items with distinct `item_category3` per property, summing exactly to the transaction `value`. Verifiers: `playwright-verify/verify-item-category3-local.mjs`, `verify-tours-item-category3-local.mjs`, `verify-multiproperty-item-category3-local.mjs`.
- **`add_to_cart`/`remove_from_cart` double-fired with two different (one wrong) values; currency casing inconsistent** (2026-07-16) — ✅ Merged to `fix/ga4-currency-value-correctness` (frontend). Every room/tour add fired **two** `add_to_cart` events: a shared cart-mutation handler using the whole cart's running total as `value` (wrong once more than one item is in the cart, and missing `items[]` entirely), plus a second, correct per-item push from the calling component (`CardRoomComponent`, tours flow). Fixed the shared handler to look up the specific added/removed line from the cart API response and build a correct single-item payload (reusing `buildGa4Item`), then removed the now-redundant duplicate pushes, eliminating the double-fire at the source. Also normalized currency to uppercase on `view_cart`/`add_to_cart`/`begin_checkout` — the raw cart-object `currency` field is stored lowercase server-side (mirrors Stripe's own convention); `purchase` already uppercased. Verified live on localhost: adding a 2nd room to a non-empty cart now fires exactly one `add_to_cart`, valued at just that room (not the cumulative cart total). Verifier: `playwright-verify/verify-add-remove-cart-local.mjs`. **Not a bug** in `purchase` itself or the backend `/confirmation` endpoint — both audited and confirmed solid (single `currency` variable reused across a whole response); see `backend/docs/CURRENCY_HANDLING_AUDIT.md` for the separate, already-fixed historical cart-currency race incident.

- **Tour coupon codes missing from `add_to_cart`/`remove_from_cart`/`purchase`** (2026-07-17) — ✅ Merged to `fix/tour-coupon-analytics` (frontend, local-only for now). Rooms already carried the applied coupon on every event; tours never did. Fixed the two single-item builders in `cartContext.tsx` (`buildAddedCartGa4Item`/`buildRemovedCartGa4Item`) to read `cart.rezdy.coupon_codes` the same way the room branch already reads `room.coupon_codes`. For `purchase` specifically, the backend confirmation response has no coupon data for tours at all (hardcoded empty `couponCodes`/`summary.coupons` in the rezdy branch — a real backend gap, deliberately deferred, not fixed here). Worked around it frontend-only: the coupon is captured into `sessionStorage` at checkout time (same established pattern already used for `hostel_country`/`hostel_name`, extended in `utils/bookingDataLayer.ts`) and read back on `/thanks`, since `purchase`'s `value` is always correct regardless (backend-confirmed) and only the coupon *label* needed a stand-in. Verified live with two real coupons on staging-backed local: `TEST10` (tour, $32.43→$29.19) and `rx1f0tzs` (room, $9.74→$8.29, confirming no regression) — both show the coupon correctly on `add_to_cart`, `begin_checkout`, and `purchase`. Verifiers: `playwright-verify/verify-tour-coupon-local.mjs`, `verify-room-coupon-local.mjs`.
- **`begin_checkout` always coupon-blind for tours (dedupe race, not yet fixed)** (2026-07-17) — found while verifying the coupon fix above, **not fixed, tracked for a separate task**. `tours-events/[slug].tsx` fires its own `begin_checkout` when "CONTINUE TO CHECKOUT" is clicked on the tour page itself — before the user ever reaches the coupon input on `/booking` — built with no `coupon` field at all. It's guarded by the same `markCheckoutEventOnce("begin_checkout", cart.id)` dedupe key as the later, correct, coupon-aware push in `booking/index.tsx` (fired on "CONFIRM & PAY"). Since the tour-page push always fires first, it wins the dedupe race and the complete one never fires — so every tour `begin_checkout` is coupon-blind regardless of whether a coupon was actually applied. Rooms don't have this problem (single-hop flow, only one `begin_checkout` push ever competes). **Impact is narrow**: `purchase`'s coupon is entirely unaffected (separate code path, verified correct — see above), and `begin_checkout`'s own `value`/`currency`/`items[]` are unaffected too — only the coupon field on that one event is missing for tours. General abandonment/funnel-drop-off reporting is not affected; only "abandonment broken down by coupon" for tours specifically would be.
